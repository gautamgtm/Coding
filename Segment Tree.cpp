...
int query_tree(int node, int a, int b, int i, int j) {
	...
	//a & b is the range of node
	if(a > b || a > j || b < i)
		return -inf; // Out of range {if interval is empty, just return from the function call}

	if(a >= i && b <= j) // Current segment is totally within range [i, j] {P.S. i, j are invariant}
		return tree[node];
